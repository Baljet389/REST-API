...
public class Board {
    ...
    private char getPieceChar(int square) {
        ...
        // White
        if ((whiteBitboards[1] & bit) != 0) return 'P';
        if ((whiteBitboards[2] & bit) != 0) return 'N';
        if ((whiteBitboards[3] & bit) != 0) return 'B';
        if ((whiteBitboards[4] & bit) != 0) return 'R';
        if ((whiteBitboards[5] & bit) != 0) return 'Q';
        if ((whiteBitboards[6] & bit) != 0) return 'K';

        // Black
        if ((blackBitboards[1] & bit) != 0) return 'p';
        if ((blackBitboards[2] & bit) != 0) return 'n';
        if ((blackBitboards[3] & bit) != 0) return 'b';
        if ((blackBitboards[4] & bit) != 0) return 'r';
        if ((blackBitboards[5] & bit) != 0) return 'q';
        if ((blackBitboards[6] & bit) != 0) return 'k';
